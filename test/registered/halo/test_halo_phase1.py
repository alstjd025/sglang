...
- JobRegistry: register_program, record_admission strict mode (Q12),
...
class TestJobRegistry(unittest.TestCase):
    ...
    # ---- Option B: record_admission strict mode ----

    def test_record_admission_rejects_unregistered_program(self):
        """Q12: lazy-create is gone. Unregistered job_id → reject."""
        reg = JobRegistry()
        result = reg.record_admission("ghost", 5.0, "rid-a")
    ...
    def test_record_admission_succeeds_after_register_program(self):
        reg = JobRegistry()
        reg.register_program("agent-1", 5.0)
        result = reg.record_admission("agent-1", 5.0, "rid-a")
    ...
    def test_record_admission_slo_conflict_keeps_pre_registered(self):
        ...
        with self.assertLogs("sglang.srt.managers.halo.job_registry", level="WARNING") as cm:
            result = reg.record_admission("agent-1", 99.0, "rid-a")
        self.assertTrue(result.admit)
    ...
    def test_gc_idle_programs_drops_unused_pre_registered(self):
        ...
        reg.record_admission("agent-2", 5.0, "rid-a")  # agent-2 has a request now
