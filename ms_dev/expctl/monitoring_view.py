...
_LAUNCH_ARG_VALUE_RE_CACHE: Dict[str, re.Pattern] = {}


def _launch_arg_regex(arg_name: str) -> re.Pattern:
    pat = _LAUNCH_ARG_VALUE_RE_CACHE.get(arg_name)
    if pat is None:
        pat = re.compile(rf"{re.escape(arg_name)}(?:\s+|=)(\S+)")
        _LAUNCH_ARG_VALUE_RE_CACHE[arg_name] = pat
    return pat


def detect_launch_arg(process_log_dir: Path, role: str, arg_name: str) -> Optional[str]:
    text = _read_file_head(process_log_dir / f"{role}.stdout.log")
    if text is None:
        return None
    m = _launch_arg_regex(arg_name).search(text)
    if m is None:
        return None
    return m.group(1).strip("'\"")


def detect_runtime_model_info(
    process_log_dir: Path,
    enabled_roles: Dict[str, bool],
) -> Dict[str, Optional[str]]:
    roles_to_probe = ("server", "prefill", "decode")
    result: Dict[str, Optional[str]] = {}
    for role in roles_to_probe:
        if enabled_roles.get(role, False):
            result[role] = detect_launch_arg(process_log_dir, role, "--model-path")
    return result
...
def render_status(
    ...
    prefill_model = model_info.get("prefill") if isinstance(model_info, dict) else None
    decode_model = model_info.get("decode") if isinstance(model_info, dict) else None
    if prefill_model and decode_model and prefill_model == decode_model:
        model_line = f"model: {colorize(prefill_model, 'accent', use_color)}"
    else:
        parts = []
        if enabled_roles.get("prefill", False):
            parts.append(f"prefill={colorize(prefill_model or 'unknown', 'accent', use_color)}")
        if enabled_roles.get("decode", False):
            parts.append(f"decode={colorize(decode_model or 'unknown', 'accent', use_color)}")
        model_line = "model: " + (" | ".join(parts) if parts else colorize("unknown", "muted", use_color))
...
def render_status_single(
    ...
    lines.append(f"session_dir={session_dir}")
    server_model = model_info.get("server") if isinstance(model_info, dict) else None
    lines.append(
        f"model: {colorize(server_model or 'unknown', 'accent', use_color)}"
    )
