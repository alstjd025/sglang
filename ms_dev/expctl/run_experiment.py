...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--router-port", type=int, default=31000)
    p.add_argument("--router-metrics-port", type=int, default=39000)
...
def main() -> int:
    ...
    ms_dev_dir = Path(__file__).resolve().parent.parent
    script_prefill = ms_dev_dir / "start_server_pd_prefill.sh"
    script_decode = ms_dev_dir / "start_server_pd_decode.sh"
    script_router = ms_dev_dir / "start_router_pd.sh"
    script_single = ms_dev_dir / "start_server_no_pd.sh"

    # Default observability for launcher-driven runs: equivalent to the old
    # sglang_start_server_request_observability.sh (metrics + request logs).
    script_args_by_role: Dict[str, List[str]] = {
        "server": ["--metrics", "--request-logs"],
        "prefill": [],  # defaults inside the script already cover metrics+request-logs
        "decode": [],
        "router": [],
    }
    ...
    request_log_root = raw_dir / "request_logs"
    request_metrics_root = raw_dir / "request_metrics"
    crash_dump_root = raw_dir / "crash_dump"
    router_log_dir = raw_dir / "router_logs"
    metrics_dir = session_dir / "metrics"
    meta_dir = session_dir / "meta"

    process_log_dir.mkdir(parents=True, exist_ok=True)
    request_log_root.mkdir(parents=True, exist_ok=True)
    request_metrics_root.mkdir(parents=True, exist_ok=True)
    crash_dump_root.mkdir(parents=True, exist_ok=True)
    ...
    if mode == "pd":
        ...
        # Propagate CLI port args to the shell scripts so non-default ports
        # actually take effect (the scripts read these env vars from env.pd.sh).
        launch_env_overrides: Dict[str, Dict[str, str]] = {
            ...
            "router": {
                ...
                "SGLANG_ROUTER_PROMETHEUS_PORT": str(args.router_metrics_port),
                "SGLANG_PD_PREFILL_PORT": str(args.prefill_port),
                "SGLANG_PD_DECODE_PORT": str(args.decode_port),
                "SGLANG_ROUTER_LOG_DIR": str(router_log_dir),
            },
        }
    else:
        role_endpoints = {
    ...
    session_paths = {
        ...
        "raw_dir": str(raw_dir),
        "request_log_root": str(request_log_root),
        "request_metrics_root": str(request_metrics_root),
        "crash_dump_root": str(crash_dump_root),
        "router_log_dir": str(router_log_dir),
        "metrics_dir": str(metrics_dir),
        "meta_dir": str(meta_dir),
        "exports_csv_dir": str(session_dir / "exports" / "csv"),
        "exports_plots_dir": str(session_dir / "exports" / "plots"),
    }
    ...
    if not args.no_auto_kill_ports:
        ...
        if cleanup_result["killed_pids"]:
            print(
                f"[run_experiment] pre-cleanup killed pids: {cleanup_result['killed_pids']}",
                flush=True,
            )
        if cleanup_result["survivors"]:
            print(
                f"[run_experiment] WARNING: ports still occupied after cleanup: {cleanup_result['survivors']}",
                flush=True,
            )

    print(f"[run_experiment] mode={mode}", flush=True)
    print(f"[run_experiment] session={session_name}", flush=True)
    print(f"[run_experiment] session_dir={session_dir}", flush=True)
    print("[run_experiment] launching processes...", flush=True)
    ...
    try:
        for role in startup_order:
            ...
            handle = launch_process(
                ...
                script_args=script_args_by_role.get(role),
            )
            handles[role] = handle
            print(f"[run_experiment] launched {role} pid={handle.proc.pid}", flush=True)
        ...
        print("[run_experiment] interrupted by user; shutting down...", flush=True)
        return_code = 0
    except Exception as e:
