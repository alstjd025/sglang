#!/usr/bin/env bash
# PD disaggregation — router (sglang_router) entry point.
set -euo pipefail
...
ensure_router_installed() {
  ...
  echo "[start_router_pd] sglang_router is missing; attempting automatic build/install..."
  apt-get update
...
# Accept any extra CLI passthrough (so users can append --no-enable-trace etc.)
if (( $# > 0 )); then
  args+=("$@")
fi

echo "[start_router_pd] launching router:"
